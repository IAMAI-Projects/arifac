      ...
      >
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
          {galleryItems.map((item, idx) => (
            <article key={item.title} className="relative border border-neutral-200 bg-white p-5 lg:p-6 overflow-hidden group hover:border-brand/30 transition-all">
              <div className="absolute top-3 right-4 text-[42px] font-black text-neutral-100 group-hover:text-brand/[0.08] transition-colors select-none pointer-events-none">
                {String(idx + 1).padStart(2, "0")}
              </div>
              <h3 className="text-[16px] font-bold text-neutral-900 mb-2 relative z-10">{item.title}</h3>
              <p className="text-[14px] text-neutral-600 leading-relaxed relative z-10">{item.text}</p>
            </article>
          ))}
        </div>
        ...
        <div className="mt-8 bg-brand text-white p-6 lg:p-8 flex flex-col lg:flex-row lg:items-center justify-between gap-5">
          ...
          <Link href="/contact" className="bg-white text-brand px-5 py-3 text-[12px] font-bold uppercase tracking-wider hover:bg-brand-dark hover:text-white transition-colors text-center">
