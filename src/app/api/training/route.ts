...
export async function POST(req: NextRequest) {
    try {
        ...
        const subjectTitle = type === 'volunteer' ? 'New Training Volunteer Application' : 'New Training Topic Request';

        await transporter.sendMail({
            from: `"ARIFAC Website" <${process.env.SMTP_USER}>`,
            replyTo: `"${firstName} ${lastName}" <${email}>`,
            to: 'help.arifac@iamai.in',
            subject: `[ARIFAC] ${subjectTitle}`,
            html: `
                <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
                    <div style="background: #1a1a2e; padding: 24px 32px; border-radius: 8px 8px 0 0;">
                        <h2 style="color: #ffffff; margin: 0; font-size: 20px;">${subjectTitle}</h2>
                        ...
                        <table style="width: 100%; border-collapse: collapse;">
                            <tr>
                                ...
                                <td style="padding: 10px 0; border-bottom: 1px solid #e5e7eb;">
                                    <span style="font-size: 14px; color: #111827;">${firstName} ${lastName}</span>
                                </td>
                            </tr>
                            <tr>
                                <td style="padding: 10px 0; border-bottom: 1px solid #e5e7eb;">
                            ...
                            <tr>
                                <td style="padding: 10px 0; border-bottom: 1px solid #e5e7eb;">
                                    <span style="font-size: 12px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.05em; color: #6b7280;">Mobile</span>
                                </td>
                                <td style="padding: 10px 0; border-bottom: 1px solid #e5e7eb;">
                            ...
                            <tr>
                                <td style="padding: 10px 0; border-bottom: 1px solid #e5e7eb;">
                                    <span style="font-size: 12px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.05em; color: #6b7280;">Agreement</span>
                                </td>
                                <td style="padding: 10px 0; border-bottom: 1px solid #e5e7eb;">
                                    <span style="font-size: 14px; color: #111827;">${agreement ? 'Confirmed' : 'Not Confirmed'}</span>
                                </td>
                            </tr>
                        </table>
                        <div style="margin-top: 24px;">
                            <p style="font-size: 12px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.05em; color: #6b7280; margin: 0 0 10px;">${type === 'volunteer' ? 'Expertise & Topics' : 'Proposed Topic'}</p>
                            <div style="background: #ffffff; border: 1px solid #e5e7eb; border-radius: 6px; padding: 16px 20px;">
                                <p style="font-size: 14px; color: #374151; margin: 0; line-height: 1.7; white-space: pre-wrap;">${topics}</p>
                            </div>
                        </div>
                    </div>
                </div>
